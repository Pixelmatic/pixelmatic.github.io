...
Maybe also define our cat:

```csharp
struct Cat: IComponentData 
{
  int hungry;         // 0: not hungry, 100: hungry to death
  int tiredness;      // 0: not tired, 100: tired to death
  ActionType action;  // current action to perform
}
```
...
class EatActionSystem: SystemBase 
{
  void OnUpdate() 
  {
    ...
    Entities.ForEach((ref Cat cat, in EatAction eatAction) => {
      cat.hunger -= eatAction.hungerRecoverPerSecond * deltaTime;
    }).ScheduleParallel();
  }
}
    ...
    Entities.ForEach((ref PlayScore playScore, in Cat cat) => {
      ...
      var eatConcern = 
        ResponseCurve.Exponential(
            math.clamp(cat.hunger * 0.01f, 0f, 1f)
        );
      
      var sleepConcern = 
        ResponseCurve.RaiseFastToSlow(
            math.clamp(cat.tiredness * 0.01f, 0f, 1f)
        );
